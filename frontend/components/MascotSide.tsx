...
import mascoteImg from '../assets/mascote_braguinho2.png';
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-4 lg:p-8 space-y-6">
      <div className={`relative transition-all duration-700 ${isThinking || isSpeaking ? 'scale-110' : 'scale-100'}`}>
        ...
        <div className={`absolute inset-0 blur-[80px] opacity-40 rounded-full transition-all duration-1000 ${isSpeaking
      ...
      <div className={`relative bg-white p-6 rounded-[2.5rem] rounded-tl-none shadow-2xl border-4 transition-all duration-500 ${isSpeaking ? 'border-red-500 ring-4 ring-red-100' : 'border-yellow-400'
        } max-w-sm text-center transform hover:scale-105 active:scale-95 cursor-default`}>
        <div className={`absolute -left-5 top-0 w-10 h-10 bg-white border-l-4 border-t-4 ${isSpeaking ? 'border-red-500' : 'border-yellow-400'
          } -rotate-45`}></div>
